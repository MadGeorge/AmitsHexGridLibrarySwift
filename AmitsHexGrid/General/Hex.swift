...
public struct Hex {
    ...
    /// Cube coordinate system initialiser
    public init(q: Int, r: Int, s: Int) throws {
        ...
        if (q + r + s) != 0 {
            throw IncorrectArguments(message: "Sum of coordinates should be zero")
        }
    }
    
    /// Axial coordinate system initialiser
    public init(q: Int, r: Int) throws {
        try self.init(q: q, r: r, s: -q - r)
    }
    
    public struct IncorrectArguments: Error {
        var message: String
    }
}
...
extension Hex {
    ...
    public func add(hex: Hex) -> Hex {
        return try! Hex(q: q + hex.q, r: r + hex.r, s: s + hex.s)
    }
    ...
    public func multiply(by k: Int) -> Hex {
        return try! Hex(q: q * k, r: r * k, s: s * k)
    }
}
...
extension Hex {
    ...
    public func distance(to hex: Hex) -> Int {
        return self.subtract(hex: hex).length
    }
}
